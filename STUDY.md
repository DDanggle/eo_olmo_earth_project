...
### 2026-09-22 개념 카드 — 이미지 교체와 정답이 바뀌는 반사실은 다르다

- 부딪힌 곳: MS-155의 wrong-content는 반대 사분면 영상으로 바꿨지만 그 사분면의 정답을
  확인하지 않고 원래 gold로 채점했다. 두 공간에서 같은 달 변화가 있을 가능성을 제거하지 않았다.
- 정답률이 비슷해도 개별 답은 달라질 수 있다(Qwen11/22 답 변경). 비슷한 집계만으로 이미지가
  무시됐다고 단정하지 않는다. 정답이 바뀌어야 하는 쌍은 대체 입력의 정답과 관측 가능성을 먼저 확인한다.
- Q2의 누적8동 도달월과 ‘처음 보인 달’도 다른 목표다. 정답 정의가 틀리면 SFT가 이를 해결하기보다
  더 잘 외울 수 있다. gate 실패 유지와 실패 원인의 제한적 해석은 양립한다.
- 확인 질문: ‘엉뚱한 영상에도 같은 정확도면 이미지 무시가 증명되는가?’ 아니다. 대체 영상의 실제
  정답, 항목별 출력 변화, text-only/null 대조, 목표 정의를 확인해야 한다.
- 근거: `code/sn7_diag_lite_v0_4.py:55`, `code/audit_sn7_v04_interpretation.py`,
  `docs/MS155_READER_AND_EXPERT_TRANSFER_REVIEW_2026_09_22.md`.

### 2026-09-22 개념 카드 — 전문가가 붙인 정답과 모델에게 보이는 근거는 다르다

- 부딪힌 곳: 공개 전문가 주석을 재사용해 비용을 줄이려 했지만 ForestNet 판독자는 고해상도
  참조를 보고, 모델 입력은 다른 해상도/시기의Landsat이다. 최종주석을 이전시점 정답으로 복사할 수 없다.
- 원래 과제의mask/class는 학습 감독으로 활용하되, 당시 제공영상의 판독가능성과 첫확인시점은
  별도 계약이다. mask에서 생성한 문장도 자동silver이지 전문가의 해설·사고과정이 아니다.
- v0.5에서 같은 ‘정보부족’에 합의했어도 한 명은기준구름, 다른명은다른영상애매함을 근거로 들면
  한 사람의 근거를 합의gold로 내보내지 않는다. 답·근거·관측원입력의 합의를 분리 확인한다.
- 확인 질문: ‘전문가가 최종홍수mask를 만들었으니 홍수전영상에도 같은언어정답을 붙여도 되나?’
  안 된다. 후영상에서알게된사실과 그prefix에서관측가능한증거는 다르며, 이는 미래정보누수다.
- 근거: `docs/EXPERT_LABELED_NATURE_DATA_2026_09_22.md`, `code/sn7_visible_contract_v05.py`.
