...
> **OlmoEarth Studio(플랫폼) 제품 작업은 2026-09-24부터 `../olmoearth_platform`에서 관리한다.** 이 저장소는 연구(CVPR·임베딩)만.
...
**그러나 task-risk와 router의 유효성은 아직 0% 측정 상태다.** 6/6 게이트가 보장하는 범위는
공간 분할의 성립까지이며, downstream 성능·CVPR 기여가 검증됐다는 뜻이 아니다.

test는 "untouched"가 아니라 **"dataset-only audit 후, downstream 학습·평가 이전에 동결"**이다.
split 규칙은 클래스 분포를 본 뒤 세 차례 수정됐고 그 사실은 M10에 기록돼 있다.

희소 클래스의 실질 독립 표본은 타일이 아니라 **군집** 수다(test 기준 벌목 5군집·산사태 3군집).
