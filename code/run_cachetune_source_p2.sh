#!/usr/bin/env bash
# CacheTune PT-1 준비 — 개발 폴드(holdout_china, holdout_chimanimani)의 source P2 decoder를 봉인 pilot 스냅샷으로 학습.
# 스냅샷을 만들고 **그 사본을 실행**한다(run_c1b 와 같은 규약). GPU1 only.
set -euo pipefail
ROOT=/home/work/data/olmoearth; PY="$ROOT/.venv-master/bin/python"
OUTROOT="$ROOT/cachetune_source_p2_v1"
...
for fold in holdout_china holdout_chimanimani; do
  for seed in 1 2 3; do
    ...
    echo "=== source P2 $fold seed=$seed $(date -u +%H:%M:%S) ==="
    env PYTHONPATH="$OUTROOT/code_snapshot" CUDA_VISIBLE_DEVICES=1 "$PY" "$OUTROOT/code_snapshot/pilot_sen12_gp_heads.py" \
      --cache "$CACHE" --emb-cache "$CACHE" --fold "$fold" --arms P2 --seed "$seed" --save-probs --out "$out"
  done
done
echo ALL_DONE
