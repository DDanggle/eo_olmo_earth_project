...
class Projector(nn.Module):
    def __init__(s):
        super().__init__(); s.mlp=nn.Sequential(nn.Linear(768,2048),nn.GELU(),nn.Linear(2048,H)); s.ttype=nn.Embedding(6,H); s.norm=nn.LayerNorm(768); s.out=nn.LayerNorm(H); s.gain=nn.Parameter(torch.tensor(1.0)); s.date=nn.Linear(3,H); nn.init.normal_(s.ttype.weight,std=0.02); nn.init.zeros_(s.date.weight); nn.init.zeros_(s.date.bias)
    def forward(s,tokens,types,datef=None):
        e=s.out(s.mlp(s.norm(tokens)))*(EMB_RMS*s.gain)+s.ttype(types)*EMB_RMS
