...
for ep in range(a.epochs):
    ...
    for j in order:
        ...
        # temporal contrast: weight pre-event steps of positive tiles so that pre and post steps contribute equally
        Wt=torch.ones(U.shape[0],1,1,1,device=dev)
        if post is not None and 0<post<U.shape[0] and FULLMASK[s].any(): Wt[:post]=(U.shape[0]-post)/post
        logit=lh(X); loss=(F.binary_cross_entropy_with_logits(logit,Y,reduction="none")*Wt).mean(); opt.zero_grad(); loss.backward(); opt.step(); tot+=float(loss)
...
res={"schema":"p1-causal-update-v0.2","split":{k:len(v) for k,v in split.items()},"far_target":a.far,"arms":{}}
