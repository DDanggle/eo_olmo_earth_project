...
def states_for(arm,model,s):
    ...
    if arm.endswith("+head"): return states_for(arm[:-5],models.get(arm[:-5]),s)
...
# matched-head training: same recipe on each arm's state sequence
FULLMASK={s:torch.from_numpy((np.load(SEALED/"mask_u8"/f"{s}.npy")>0).astype("float32")) for s in ids}
def train_head(arm):
    h=LatestHead().to(dev); opt=torch.optim.AdamW(h.parameters(),1e-3,weight_decay=1e-4)
    for ep in range(a.epochs):
        order=np.random.permutation(len(split["train"])); tot=0
        for j in order:
            s=split["train"][j]; S=states_for(arm,models.get(arm),s).detach(); post=META[s]["post_index"]; X=((S-mean)/sd); Y=FULLMASK[s].to(dev)[None,None].expand(S.shape[0],1,128,128).clone()
            if post is not None: Y[:post]=0
            else: Y[:]=0
            Wt=torch.ones(S.shape[0],1,1,1,device=dev)
            if post is not None and 0<post<S.shape[0] and FULLMASK[s].any(): Wt[:post]=(S.shape[0]-post)/post
            logit=h(X); loss=(F.binary_cross_entropy_with_logits(logit,Y,reduction="none")*Wt).mean(); opt.zero_grad(); loss.backward(); opt.step(); tot+=float(loss)
        print(f"head[{arm}] epoch {ep} loss {tot/len(order):.4f} {time.perf_counter()-t0:.0f}s",flush=True)
    h.eval(); torch.save(h.state_dict(),OUT/f"head_{arm}_seed{a.seed}.pt"); return h
HEADS={}
for arm in ("last","gru_dt","prefix","resid_q"): HEADS[arm]=train_head(arm)
lh=HEADS["last"]
SEALED_EMB={s:(SEALED/"emb_fp16"/f"{s}.npy") for s in ids}
ARMS=["last","latest_head","gru_dt","gru_dt+head","prefix","prefix+head","resid_q","resid_q+head","meta_only","sealed12"]
@torch.no_grad()
def readout_area_iou(S,s,arm="x"):
    X=(S-mean)/sd; net=(lh if arm=="latest_head" else HEADS[arm[:-5]] if arm.endswith("+head") else dec); P=torch.sigmoid(net(X).float()).squeeze(1).cpu().numpy()[:,W,W]>0.5; Y=MASK[s]
...
res={"schema":"p1-causal-update-v0.3","split":{k:len(v) for k,v in split.items()},"far_target":a.far,"arms":{}}
...
def pre_max_areas(R): return np.array([max(x["area"][:x["post"]]) for x in R if x["post"] is not None and x["post"]>0])  # tile-level: max pre-event area per tile
def evaluate(arm,far):
    pa=pre_max_areas(rows[arm]["val"]); thr=float(np.quantile(pa,1-far)) if len(pa) else 0.0
    T=rows[arm]["test"]; pre=pre_max_areas(T); realised=float((pre>=thr).mean()) if len(pre) else None; latA=[];latB=[]; conf=0; npos=0; post_iou=[]
    for x in T:
    ...
    if arm=="sealed12":
        pos_above=[x["area"][0]>=thr for x in T if x["positive"]]; neg_above=[x["area"][0]>=thr for x in T if not x["positive"]]
        return {"far_budget":far,"thr_area_px":thr,"readout_sanity_positive_frac_above":float(np.mean(pos_above)) if pos_above else None,"negative_tiles_frac_above":float(np.mean(neg_above)) if neg_above else None,"n_positive":len(pos_above),"n_negative":len(neg_above),"note":"one decision per tile; no latency"}
    return {"far_budget":far,"thr_area_px":thr,"test_pre_event_far":realised,"head_valid":(realised is not None and realised<3*far) if (arm=="latest_head" or arm.endswith("+head")) else None,"n_positive":npos,"confirmed":conf,"never_confirmed":npos-conf,"confirm_rate":conf/max(npos,1),"latencyA_event_to_confirm_median":float(np.median(latA)) if latA else None,"latencyA_p25_p75":q(latA),"latencyB_firstvalidpost_to_confirm_median":float(np.median(latB)) if latB else None,"latencyB_p25_p75":q(latB),"post_event_mean_iou":float(np.mean(post_iou)) if post_iou else None,"encode_units_per_arrival":"min(c,12)" if arm=="prefix" else ("12 (full year, non-causal reference)" if arm=="sealed12" else 1)}
for arm in ARMS:
...
def iou_map(arm):
    thr=res["arms"][arm]["far10"]["thr_area_px"]; out={}
    for x in rows[arm]["test"]:
        if not x["positive"] or x["post"] is None: continue
        out[x["id"]]=float(np.mean(x["iou"][x["post"]:]))
    return out
for other in ("gru_dt+head","prefix+head","resid_q+head"):
    A,B=iou_map(other),iou_map("latest_head"); common=[k for k in A if k in B]; d=np.array([A[k]-B[k] for k in common]); bs=[rng.choice(d,len(d)).mean() for _ in range(2000)] if len(d)>5 else []
    LA,LB=lat_map(other) if False else None,None
    res["paired"][f"{other}_minus_latest_head_post_iou"]={"n":len(common),"mean":float(d.mean()) if len(d) else None,"ci95":[float(np.quantile(bs,.025)),float(np.quantile(bs,.975))] if bs else None}
