...
env -u PYTHONPATH CUDA_VISIBLE_DEVICES=1 $PY code/extract_galileo_cache.py --size base --patch 4 --out galileo_cache > logs/galileo_cache.log 2>&1; echo "$(date -u +%FT%TZ) galileo extract rc=$?" >> $LOG
