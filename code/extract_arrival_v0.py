...
ap=argparse.ArgumentParser(); ap.add_argument("--out",default=None); ap.add_argument("--region",default="hiroshima"); ap.add_argument("--size",default="base",choices=["nano","tiny","base"]); ap.add_argument("--probe",action="store_true"); ap.add_argument("--data-root",default="/home/work/data/sen12landslides/extracted"); a=ap.parse_args()
...
ROOT=Path("/home/work/data/olmoearth"); OUT=ROOT/(a.out or (f"arrival_v0/{a.region}" if a.size=="base" else f"arrival_v0_{a.size}/{a.region}")); dev=torch.device("cuda"); PATCH=4
CH={"nano":128,"tiny":192,"base":768}[a.size]  # channel count fixed here: inside window() the name a is the crop offset, not argparse
...
ids=sorted(s for s,r in REC.items() if r["region"]==a.region and not r.get("error"))
for d in ("single_fp16","prefix_fp16","meta"): (OUT/d).mkdir(parents=True,exist_ok=True)
w=OlmoEarth(patch_size=PATCH, model_id={"nano":ModelID.OLMOEARTH_V1_NANO,"tiny":ModelID.OLMOEARTH_V1_TINY,"base":ModelID.OLMOEARTH_V1_BASE}[a.size], token_pooling=True, use_legacy_timestamps=False, normalize=True, autocast_dtype="bfloat16").to(dev).eval()
