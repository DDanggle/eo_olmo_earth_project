...
import json, re, sys, time, hashlib, numpy as np
from pathlib import Path
ROOT=Path("/home/work/data/olmoearth"); SN7=ROOT/"spacenet7/train"; OUT=ROOT/"spacenet7/diag_lite_v0_4"; OUT.mkdir(parents=True,exist_ok=True)
CUTOFFS=[12,20]; K=4; QUADS=["NW","NE","SW","SE"]; WIN=6; MIN_NEW=8; OPP={"NW":"SE","SE":"NW","NE":"SW","SW":"NE"}
        ...
        for cut in CUTOFFS:
            ...
            for q in QUADS:
                ...
                for m in pre[1:]:
                    ...
                    if cum>=MIN_NEW: reach.append((pre.index(m),q,m)); break
            reach=[r for r in reach if r[0]>=2]  # v0.4: gold index >= 2 so the answer is not (almost) always the 2nd month
            if reach: _,qq,mm=max(reach); q2=(qq,mm)  # latest-reaching quadrant spreads gold over the series
            ...
            def conds(gold_month,q1_window=None):
                ...
                if gold_month and gold_month in pre:
                    ...
                    else:  # v0.4: gold position uniform in {1,2,3}; k frames before (spread), 3-k clear frames after
                        seed=int(hashlib.sha256(f"{aoi}|{cm}|{gold_month}".encode()).hexdigest(),16)%3; k=1+seed
                        before_pool=pre[:gi]; after_pool=sorted(pre[gi+1:],key=lambda m:unus.get(m,0.0))
                        k=min(k,len(before_pool)); na=3-k
                        if len(after_pool)<na: na=len(after_pool); k=min(3-na,len(before_pool))
                        before=[before_pool[int(round(x))] for x in np.linspace(0,len(before_pool)-1,k)] if k>0 else []
                        c["privileged_silver"]=sorted(set(before+[gold_month]+after_pool[:na]))
                        c["privileged_wrongcontent"]=c["privileged_silver"]
...
def run():
    ...
    for it in items:
        for cond,frames in it["conds"].items():
            ...
            qsrc=OPP[it["region"]] if cond=="privileged_wrongcontent" else it["region"]; paths=[it["png"][m][qsrc] for m in frames]; text=prompt(it,frames).replace("<video> ","")
            content=[]
...
def score():
    ...
    ids=[i for i in {r["id"] for r in rows if r["q"]=="Q2"} if (i,"privileged_silver") in byk and (i,"privileged_wrongcontent") in byk]
    if ids:
        d={i:float(byk[(i,"privileged_silver")]["pred"]==byk[(i,"privileged_silver")]["gold"])-float(byk[(i,"privileged_wrongcontent")]["pred"]==byk[(i,"privileged_wrongcontent")]["gold"]) for i in ids}
        per_aoi={a:np.mean([d[i] for i in ids if i.startswith(a+"|")]) for a in aois if any(i.startswith(a+"|") for i in ids)}; v=np.array(list(per_aoi.values())); bs=[rng.choice(v,len(v)).mean() for _ in range(5000)]
        out["content_check_Q2"]={"n_items":len(ids),"diff":float(v.mean()),"ci95":[float(np.percentile(bs,2.5)),float(np.percentile(bs,97.5))],"pass":bool(v.mean()>=.30 and np.percentile(bs,2.5)>0)}
    gpos={}
    for i in {r["id"] for r in rows if r["q"]=="Q2"}:
        pass
