...
rc=$?; if [[ $rc -ne 0 ]]; then log "manifest FAILED rc=$rc"; touch "$L/t1_FAILED.txt"; exit "$rc"; fi
wait_free; env -u PYTHONPATH CUDA_VISIBLE_DEVICES=1 $PY code/extract_olmo_streaming.py --out olmo_streaming_dev --ids-file sen12_gp_contract/t1_ids.txt > "$L/x_olmo_streaming.log" 2>&1
rc=$?; log "extraction rc=$rc"
if [[ $rc -ne 0 ]]; then touch "$L/t1_FAILED.txt"; exit "$rc"; fi
$PY code/validate_temporal_cache_evidence.py olmo_streaming_dev/olmo_streaming_audit.json --minimum-examples 30 || { log "streaming audit FAILED"; touch "$L/t1_FAILED.txt"; exit 5; }
...
for fold in holdout_chimanimani holdout_hiroshima; do for mod in ema gru residual; do for s in 1 2 3; do
  ...
  env -u PYTHONPATH CUDA_VISIBLE_DEVICES=1 $PY code/streaming_update_train.py --fold $fold --module $mod --seed $s --out artifacts/streaming_t1 > "$L/t1_${fold}_${mod}_s${s}.log" 2>&1
  rc=$?; log "t1 $fold $mod s$s rc=$rc"
  if [[ $rc -ne 0 ]]; then touch "$L/t1_FAILED.txt"; exit "$rc"; fi
done; done; done
