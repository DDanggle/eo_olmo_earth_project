...
for m in man:
    ...
    pl={"SENTINEL-2A":"S2A","SENTINEL-2B":"S2B"}.get(plat,plat)
    if plat and cands and not all(c.startswith(pl) for c in cands): ok=False; why.append(f"platform {plat} vs {cands[0][:3]}")
