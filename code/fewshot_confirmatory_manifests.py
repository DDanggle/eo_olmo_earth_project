#!/usr/bin/env python3
"""확증 8지역 target support/query 공간분리 manifest — PT-0 규칙 그대로(fewshot_a1_vs_a4_prereg_v0.confirmatory_protocol_registered_now) (CPU, 모델 지표 미열람).
...
OUT=Path("/home/work/data/olmoearth/artifacts/fewshot_confirmatory_manifests"); OUT.mkdir(parents=True,exist_ok=True)
REGIONS=["hiroshima","hokkaido","indonesia","itogon","kyrgyzstan1","kyrgyzstan2","newzealand","thrissur"]; KS=[5,20]; BLOCK_M=10_000; BUFFER_M=3_000; POS_FRAC_SUPPORT=0.40
def sha(o): return hashlib.sha256(json.dumps(o,sort_keys=True).encode()).hexdigest()
rep={"schema":"fewshot-confirmatory-manifest-v2","assignment_rule":"v2: pool positive cap 45% (block-level knapsack in x order); changed before any metric was read because v1 gave hokkaido a zero-positive query","rules":__doc__,"regions":{}}
for region in REGIONS:
    ...
    for f in sorted(glob.glob(str(NC/f"{region}_s2_*.nc"))):
        ...
        mp=CACHE/"mask_u8"/f"{sid}.npy"
        if not mp.exists(): continue   # S12q 계약 밖 타일(캐시 없음)은 실험 우주에 없음
        m=np.load(mp); pos=int(m.sum())
    ...
    # 배정 규칙 v2 (2026-09-02, 지표 열람 전 변경): x 순서로 훑되 블록을 pool 에 넣었을 때 pool 양성이 전체의 45% 를 넘으면 query 로.
    # v1(40% 도달까지 무조건 pool)은 hokkaido 처럼 한 블록에 양성이 몰린 지역에서 query 양성 0 을 만들었다.
    for k in order:
        bp=sum(t["positive"] for t in blocks[k])
        if acc+bp <= 0.45*total_pos and acc < POS_FRAC_SUPPORT*total_pos: pool+=blocks[k]; acc+=bp
        else: query+=blocks[k]
    # buffer
