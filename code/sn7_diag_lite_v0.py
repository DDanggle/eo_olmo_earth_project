#!/usr/bin/env python3
"""G1-lite bottleneck diagnostic on SpaceNet 7 (config/bottleneck_diag_lite_prereg_v0.json).
...
ROOT=Path("/home/work/data/olmoearth"); SN7=ROOT/"spacenet7/train"; OUT=ROOT/"spacenet7/diag_lite_v0"; OUT.mkdir(parents=True,exist_ok=True)
CUTOFFS=[12,20]; K=4; QUADS=["NW","NE","SW","SE"]; WIN=6; MIN_NEW=3
...
def build():
    ...
    for aoi in dev_aois():
        ...
        for p,m in zip(imgs,months):
            ...
            o=fr_dir/f"{m}.png"
            if not o.exists(): Image.fromarray(a.astype("uint8")).resize((512,512)).save(o)
            png[m]=str(o); arrs[m]=a[::4,::4].astype("float32").mean(2)
        unus={}
        ...
        for cut in CUTOFFS:
            ...
            def conds(gold_month):
                ...
                if gold_month and gold_month in pre:
                    gi=pre.index(gold_month); later=sorted([m for m in pre[gi+1:]],key=lambda m:unus.get(m,0.0))[:2]
                    c["privileged_silver"]=sorted(set([pre[max(gi-1,0)],gold_month]+later))  # months without UDM file are fully usable (unus default 0)
                else: c["privileged_silver"]=None
                return c
            for q in (yes[:1]+no[:1]):
                items.append({"id":f"{aoi}|{cm}|Q1|{q}","aoi":aoi,"cutoff":cm,"q":"Q1","region":q,"window":[pre[-WIN],cm],"gold":"yes" if q in yes else "no","conds":conds(None if q not in yes else next((m for m in pre[-WIN:] if new_by_mq.get((m,q),0)>0),None))})
            if q2: items.append({"id":f"{aoi}|{cm}|Q2|{q2[0]}","aoi":aoi,"cutoff":cm,"q":"Q2","region":q2[0],"gold":q2[1],"conds":conds(q2[1])})
            if q3: items.append({"id":f"{aoi}|{cm}|Q3","aoi":aoi,"cutoff":cm,"q":"Q3","gold":q3,"conds":conds(None)})
        for it in items:
...
def prompt(it,frames):
    head=f"These are {len(frames)} monthly satellite images of the same area in chronological order, taken in {', '.join(frames)}: <video> "
    if it["q"]=="Q1": return head+f"Did new buildings appear in the {it['region']} quadrant between {it['window'][0]} and {it['window'][1]}? Answer with yes or no."
    if it["q"]=="Q2": return head+f"In the {it['region']} quadrant, in which month did new construction first become clearly visible? Answer with the month in YYYY-MM format from the list above."
...
def run():
    ...
    for it in items:
        for cond,frames in it["conds"].items():
            ...
            imgs=[it["png"][m] for m in frames]
            try: ans=run_inference_single(model,proc,tok,prompt(it,frames),imgs,timestamps=[f"{m}-15" for m in frames],max_new_tokens=32)  # TEOChat wants %Y-%m-%d; monthly mosaics -> day 15
...
def score():
    ...
    for c in conds:
        out["per_condition"][c]={q:acc_rows([r for r in rows if r["cond"]==c and r["q"]==q]) for q in ("Q1","Q2","Q3")}
