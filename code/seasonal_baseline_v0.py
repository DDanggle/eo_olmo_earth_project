...
ROOT=Path("/home/work/data/olmoearth"); OUT=ROOT/"seasonal_baseline_v0"; OUT.mkdir(exist_ok=True); dev=torch.device("cuda" if torch.cuda.is_available() else "cpu"); rng=np.random.default_rng(0)
...
half=lambda s: int(hashlib.sha256(s.encode()).hexdigest(),16)%2  # 0 = A(fit), 1 = B(eval)
def auc(pos,neg):
...
res={"schema":"seasonal-baseline-v0","regions":{}}
for reg in REGIONS:
...
h=res["regions"]["hokkaido"]; ok=lambda a: a["normal_z2"] is not None and a["normal_z2"]<=.10 and a["event_auc"] is not None and a["event_auc"]>=max(.75,(h["loeo"]["event_auc"] or 0)-.03)
res["gate"]={"hokkaido_pass":bool(ok(h["inregion_season"]) or ok(h["inregion_month"])),"controls_ok":all((res["regions"][r]["inregion_season"]["normal_z2"] or 0)<=(res["regions"][r]["loeo"]["normal_z2"] or 0)+.02 for r in REGIONS if r!="hokkaido")}
