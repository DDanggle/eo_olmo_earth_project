#!/usr/bin/env python3
"""B-v1 generic frozen-cache decoder trainer (NOT the sealed pilot; a re-implementation of its P4 recipe for caches of any C x G x G).
...
from cache_grid_controls import transform_embedding
...
ap.add_argument("--grid-transform",default="native",choices=["native","upsample2","avgpool2"],help="Mechanism control: transform cached token grid before the identical decoder. upsample2 adds no representation information; avgpool2 removes patch-2 spatial detail.")
...
def emb_stats(train_ids,sample=400):
    ...
    for j in idx:
        x=transform_embedding(np.load(CACHE/"emb_fp16"/f"{train_ids[j]}.npy").astype("float32"),a.grid_transform); m=x.mean(axis=(1,2)); m2=(x**2).mean(axis=(1,2)); acc=m if acc is None else acc+m; acc2=m2 if acc2 is None else acc2+m2; n+=1
...
def load(split):
    X=torch.from_numpy(np.stack([transform_embedding(np.load(CACHE/"emb_fp16"/f"{s}.npy").astype("float32"),a.grid_transform) for s in ids[split]])); X=(X-st[0])/st[1]
...
native_shape=list(np.load(CACHE/"emb_fp16"/f"{ids['train'][0]}.npy",mmap_mode="r").shape)
rep={"schema":"cache-decoder-train-v2","cache":a.cache,"fold":a.fold,"seed":a.seed,"grid_transform":a.grid_transform,"native_emb_shape":native_shape,"emb_shape_after_transform":[C,G,G],"trainable_params":npar,"pos_weight":pw,"epochs":a.epochs,"best_val_epoch":best["epoch"],"best_val_iou":best["val_iou"],"val":mv,"test":mt,"split":{k:len(v) for k,v in ids.items()},"train_s":time.perf_counter()-t0,"history":hist,
     "recipe":"AdamW 1e-3 wd 1e-4 cosine, batch 16, BCE pos_weight exact cap 50, best val pixel IoU@0.5; identical decoder across native and token-grid controls"}
