#!/usr/bin/env python3
"""GeoContextGate v2 — 타깃 지역 라벨 없이 "이 타일은 embedding arm(P4)을 믿을까, raw arm(P2)을 믿을까"를 예측.

사전 등록: config/geocontextgate_impl_v2.json (v1 실패 진단 반영: 게이트 이전 arm별 FP 예산 정합), 승급 기준: config/geocontextgate_promotion_gate.json.
...
OUT=Path("/home/work/data/olmoearth/artifacts/geocontextgate_v2"); OUT.mkdir(parents=True, exist_ok=True)
...
rep={"schema":"geocontextgate-v2","preregistration":"config/geocontextgate_impl_v2.json",
...
for s in (1,2,3):
    ...
    for target in REGIONS:
        ...
        d=data[(target,s)]
        # --- v2: source 지역 empty tile 로 arm 별 FP 예산 임계 t_a 를 구하고 logit 평행이동 (타깃 라벨 미사용)
        srcP={a:[] for a in ("P2","P4")}; srcY=[]
        for r in src:
            dr=data[(r,s)]; srcY.append(dr["y"]); srcP["P2"].append(dr["p2"]); srcP["P4"].append(dr["p4"])
        SY=np.concatenate(srcY); S2=np.concatenate(srcP["P2"]); S4=np.concatenate(srcP["P4"])
        budget_src=empty_fp(S4, SY, 0.5)                       # source 기준 P4 예산
        t2=thr_for_fp_budget(S2, SY, budget_src); t4=0.5
        shift=lambda p,t: 1/(1+np.exp(-(np.log(np.clip(p,EPS,1-EPS)/(1-np.clip(p,EPS,1-EPS))) - np.log(t/(1-t)))))
        p2c=shift(d["p2"],t2); p4c=shift(d["p4"],t4)
        Xt=np.hstack([d["F"], (d["mu"]-pm)@pv]); Xt=(Xt-mu_x)/sd_x
        alpha=predict(Xt,w,b)
        P_hard=np.where(alpha[:,None,None]>=0.5, p4c, p2c)
        P_soft=alpha[:,None,None]*p4c+(1-alpha[:,None,None])*p2c
        P_avg=(p2c+p4c)/2
        ...
        # 상한: 타일 oracle
        Po=np.where((d["iou4"]>d["iou2"])[:,None,None], p4c, p2c)
        thr_o=thr_for_fp_budget(Po,d["y"],budget)
        row["oracle_tile"]={"iou_05":pos_tile_iou_from_probs(Po,d["y"],0.5),
                            "iou_fp_matched":pos_tile_iou_from_probs(Po,d["y"],thr_o)}
        row["alpha_std"]=float(alpha.std()); row["alpha_frac_gt_half"]=float((alpha>=0.5).mean())
        row["t2_source"]=float(t2)
        res[target]=row
    ...
    rep["per_seed"][str(s)]["macro_fpm_oracle"]=macro([res[r]["oracle_tile"]["iou_fp_matched"] for r in REGIONS])
...
rep["macro_oracle_fp_matched"]=float(np.mean([rep["per_seed"][str(s)]["macro_fpm_oracle"] for s in (1,2,3)]))
rep["macro_oracle_05"]=float(np.mean([rep["per_seed"][str(s)]["macro_oracle"] for s in (1,2,3)]))
