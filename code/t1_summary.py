...
for f in sorted(glob.glob(sys.argv[1] if len(sys.argv)>1 else "artifacts/streaming_t1/*.json")):
    ...
    rows[(d["fold"],d["module"])].append({"seed":d["seed"],"teacher_ap":t,"frozen_ap":fz,"student_ap":st,"singles_ap":ds["singles_mean"]["auprc_exact"],"recovery":(st-fz)/max(t-fz,1e-9),"cos":d["agreement_c12"]["student"]["cosine"],"student_macro":ds["student"]["positive_patch_macro_iou"],"teacher_macro":ds["teacher_full_reencode"]["positive_patch_macro_iou"]})
out={}
