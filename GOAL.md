...
### 2026-08-29 (밤) — 현재 실행 감사·하류 종점·AI 회랑 UX 마감 (진행 중)

- 계획: 실행 중인 회랑 27창 baseline/s1_live 물질화·봉인·임베딩의 실제 완료율과 GPU1 상태를
  산출물·서버 로그로 대조한다. 사용자 작업과 충돌하지 않도록 실행 경로 코드는 수정하지 않는다.
- 제품: 지도 수계를 Rasuwagadhi에서 Trishuli/Galchhi의 검증 가능한 하류 종점까지 분절해
  `관측 강선`, `AI 스캔 범위`, `홍수/토석류 도달(미확정)`을 서로 다른 선·범례·claim으로 표시한다.
  강변 27창과 100창 스캔의 결과를 거리순·변화순으로 탐색하고, 강 밖 hillslope 후보는 별도 큐로
  분리해 한 화면에서 근거 이미지·관측성·AI 상태를 확인하게 한다.
- AI 경계: OLMo 768×64×64 공간 임베딩의 sealed live delta, S2-only 후보 스캔, 지형/수계
  규칙을 같은 색이나 같은 `AI detected` 표현으로 합치지 않는다. 27창 처리가 끝나기 전에는
  하류 위험·산사태 종점·홍수 도달을 모델 결과로 승격하지 않는다.
- 성공 기준: ① 27창 단계별 완료/남은 수를 실측 보고 ② 하류선 종점과 미검증 도달 경계를 5초
  안에 구별 ③ river/off-river 후보 모두 지도→증거 카드 왕복 가능 ④ AI가 실제 수행한 계산과
...
- 2026-08-30 00:25 M76: dB 재계산 후 봉인 계약 5앵커·회랑 모두 미검출(사전 등록 기준). M72 토큰 9.8% 철회.
...
### 2026-08-30 (3) — Presto 대조군(M79)
- 함: Presto 클론·EE 의존성 우회(shim 대신 try/except, 모델 코드 무수정) → 7지역 690패치 같은 계약. OlmoEarth ≥+0.03 앞섬 6/7, Alaska만 역전(−0.073).
...
### 2026-08-30 (7) — M83 언색호 수색
- 함: NDWI∩S1 같은 궤도. 광학 15% 맑음 → 레이더 급감 성분만 후보(렌데 계곡 군집). D 팝업에 3장 추가.
- 마찰: 궤도 혼합 비교가 21% 급감을 만들어 냄 → 같은 상대궤도로 고정.

### 2026-08-30 (8) — M82 평시 쌍 3개
- 함: 임계 0.2819→0.317(pooled), 후보 비율 절반, 순위 Spearman 0.94 유지. P2(몬순 시작) 0.36이 최대.
- 마찰: 전부 구름인 창의 local 집계에서 concat 오류 → 가드 후 캐시 재집계.
...
### 2026-09-06 새벽 — 프로브 실험 + GEO-Bench-2 확보 (GPU 0)
- **방향 확정 (사용자와 대화로)**: 진단 생산을 멈추고 **예측기**를 만든다. "유사도 지표가
  기능 등가를 예측 못한다"는 일반 명제는 ICLR 2023(CKA reliability) 등 **선행연구가 이미 점유**해
  기여로 쓸 수 없음을 확인했다. 남은 빈칸은 "그럼 무엇이 예측하는가"다.
- **교차모델 브리지(Clay→OlmoEarth) 5폴드 전부 사전등록 불통과.** 코사인 .79–.87,
  이웃순위 .73–.90인데 R² −.23~+.20. 무작위 사상도 이웃순위 .85. 방법으로서 사망, 기준 불변.
  (중간에 hiroshima 한 폴드만 보고 "두 공간이 이미 같다"고 과잉 진술 → 나머지 폴드에서 재현
  실패로 즉시 철회. 한 폴드로 판단하지 않는다는 교훈.)
- **라벨 없는 프로브 14캐시 (신규)**: `cache_probes.py` + 단위테스트 21개.
  사전등록 ρ≥.70 불통과 → **NO_PREDICTOR_AT_N14**. 다만
  `effective_rank` ρ=+.635 CI[+.13,+.88], `participation_ratio` ρ=+.631 CI[+.04,+.93] —
  **CI가 0을 배제**하고 차원 교란(emb_dim ρ=−.262)도 통과. 물리 프로브(NDVI/NDWI/NDBI/밝기
  복원 R²)는 Clay가 오히려 최고(.886)인데 downstream은 낮아 **판별력 없음**.
  → 물리 내용은 어느 캐시에나 들어 있고, 갈리는 것은 **표현의 유효 차원**이라는 가설.
  확증은 GEO-Bench 과업 확장(n=14→70) 뒤로 사전 지정.
- **밴드 계약 함정**: raw_u16는 (10밴드, 12시점, 128, 128)이고 **B08은 인덱스 3**이다(6 아님).
  추측했으면 NDVI를 통째로 잘못 계산했다. 단위테스트에 손계산 검증으로 박아둠.
- **rc 버그 저장소 가드 신설** (`test_chain_rc_pattern.py`): 큰따옴표 안에 `$(` 와 `$?` 가
  함께 있는 줄만 잡아 오탐 제거. 봉인 결과를 **생산한** 스크립트 15개(23건)는 HISTORICAL로
  제외 — 계보 보존이 우선이고 그 결과들은 rc가 아니라 실물 산출물로 검증됐다.
  린터가 **내가 방금 쓴 `setup_geobench2.sh`의 같은 버그를 자체 적발**해 수정.
- **GEO-Bench-2 확보 시작**: PASTIS + Fields of the World + DynamicEarthNet 확정.
  PASTIS의 S2 밴드가 우리 10밴드와 **같은 집합**(순서만 다름, band_order로 지정 가능).
  설치 걸림돌 두 개 해결 — NVIDIA 컨테이너 pip constraint(torch 고정) → `--system-site-packages`
  + `PIP_CONSTRAINT=`, numpy 2.4.6 ↔ 컨테이너 torch(NumPy 1.x 컴파일) → `numpy<2` 고정.
  기본 다운로더가 단일 연결 1.0 MB/s라 병렬 range 다운로더 신설(실측 4.3 MB/s, sha256 검증,
  재개 가능, 테스트 35개). 총 102GB, ETA 약 7시간.
- **GPU 사용 0**: GPU1에 타 사용자 작업 65.9GB/82% → 규약 4b대로 미실행. 경쟁 캐시의
  Solar 재현(단일 과업 의존이라는 최대 구멍)은 GPU가 비면 실행.
- 다음: (1) GEO-Bench 적재 검증 → (2) 프로브 확증 n=70 → (3) 경쟁 캐시 Solar 재현.

### 2026-09-06 09:40 — 아침 점검: 7시간 손실의 원인과 재발 방지
- **사고**: 02:31 fotw 검증(`geobench_verify_one.py`)이 존재하지 않는 모달리티 `rgb`를 넘겨
  실패 → fail-closed 체인이 멈춤 → pastis(53GB)·DEN(44GB) 다운로드가 시작조차 못 함.
  09:27 발견, 즉시 재시작. **데이터는 처음부터 멀쩡했고 검증기 버그 + 체인 설계 실수**였다.
- **두 가지 원인**: (1) 검증기가 `band_default_order`가 dict인 경우만 가정(fotw는 tuple, 모달리티
  `self`). (2) 독립적인 다운로드를 검증 뒤에 직렬로 묶어 검증 실패가 다운로드까지 막음.
- **실손실**: 밤 대역폭 4.3 MB/s 기준 7시간이었으나 아침 22 MB/s(피크)·~11 MB/s(평균)로
  회복돼 실제 지연은 약 1시간. 그래도 설계 실수는 사실.
- **재발 방지 3종**:
  (a) `code/preflight.py geobench` — 비싼 단계 앞 5초 예비검사. 3 데이터셋 전부 통과 확인.
      이 검사가 있었으면 fotw 버그는 다운로드 전에 잡혔다.
  (b) 설계 규칙 신설: 다운로드·추출·학습처럼 **서로 독립인 비싼 단계는 한 사슬에 묶지 않는다**.
      검증은 별도 단계. (RESTART_HERE 상단에 명시)
  (c) `code/status.sh` — 한 명령으로 GPU·체인·마커·데이터·보호파일을 본다. 어느 세션에서든
      "지금 뭐가 죽었나"를 5초에 확인 가능.
- **발견**: fotw는 논문 표("S2/Multi")와 달리 **RGB+NIR 4밴드**만 노출. 3개 중 다중밴드는
  PASTIS(10밴드, 우리와 동일 집합)와 DEN(12밴드)뿐. fotw는 계약 약함으로 표기.
...
- **(추가 15:10~16:00) 학습 결과**: 폴러(`code/studio_models_poll.py`, 카드 UI라 table 셀렉터 실패 → DOM 프로브 후
  텍스트 패턴으로 수정; 빈 파싱을 완료로 오판한 초판 결함도 수정). 네팔 `audit-nano-rasuwa-status` **41분 후 failed,
  원인 표시 없음**(Error 행·툴팁·카드 메뉴·Dashboard·queue 전부 무언 → 결함 5 P0; 취소 버튼 없음 → 결함 6 P2).
  제주 `audit-nano-oreum-polys-cat-v2` **ready**: Acc 81.0% / F1 78.9% / val 84창(다수 기준선 61.9%).
  **수치의 의미**: 라벨 scored/abstain = 우리 파이프라인의 기권 플래그(구름 대리변수) → 오름 변화와 무관, 연기 테스트.
