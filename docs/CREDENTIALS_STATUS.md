...
### 산출물별 유효 시간대 — 3시간 격자는 잘못된 설계였음

2026-08-24분을 시각별로 검사해서 고쳤음. **AOD·COT·CT는 태양광 반사가 필요해 야간에는
격자 전체가 -9999임.** 3시간 격자로는 주간 산출물을 8슬롯 중 3개만 잡고 있었음.

유효 픽셀 비율 (%), KST:

| | 00 | 06 | 08 | 10 | 12 | 14 | 16 | 18 | 20 | 22 |
|---|---|---|---|---|---|---|---|---|---|---|
| **CLD** | 100 | 100 | 100 | 100 | 100 | 100 | 100 | 100 | 100 | 100 |
| **FOG** | 100 | 100 | 100 | 100 | 100 | 100 | 100 | 100 | 100 | 100 |
| AOD | 0 | 0 | 32 | 31 | 27 | 25 | 21 | 20 | — | — |
| COT | 0 | 0 | 53 | 55 | 60 | 65 | 68 | 63 | — | — |
| CT | 0 | 0 | 60 | 59 | 64 | 69 | 74 | 66 | — | — |

06시가 0%인 것은 태양고도가 낮아 산출이 실패하기 때문임. 그래서 주간 창을 **08~18시**로 잡았음.
...
받는 것은 **2 km 격자 320×397 = 127,040개 값** × 5종임. 구름탐지는 값이 0/1/2 세 개뿐이라
254 KB 원문이 12 KB로 압축됨. **영상(radiance)이 아님.**

받지 않는 것: 원본 천리안 2A 영상 — 16채널, 0.5~2 km, 2~10분 간격. 이건 다른 아카이브
(기상자료개방포털·apihub)에 있고 테라바이트급임. **그리고 그건 기상청이 장기 보관하므로
우리가 모을 이유가 없음.**

**이 자산의 가치는 용량이 아니라 `2일 보존`에서 옴.** 원본 영상은 누구나 나중에 받을 수 있지만,
이 경량화 산출물의 과거 이력은 아무도 갖고 있지 않음. 그게 희소성임.
