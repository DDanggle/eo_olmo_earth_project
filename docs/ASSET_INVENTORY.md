...
- **OLMo v1 smoke 6/6 통과**: 10지역·양/음성 32/32, 64표본/256crop, 15.44초(4.146 sample/s),
  ...
  headline 6,834개 단순 외삽은 약 27.5분 / 10.75 GB다.
...
| 측정 M-항목 | **22** |
| 코드 파일 | 90 |
| 테스트 파일 / 통과 | 18 / **153 passed, 1 skipped, 10 subtests** |
| artifacts | 19 |
| docs | 8 |
| STUDY 카드 | 53 |
| PR 후보 | 12 |
...
| **frozen OLMo probe 결과** | **1 (G-P)** | embedding smoke 통과. **성능 head/baseline은 아직 0** |
| **matched head/baseline 1 run 시간** | **G-C** | embedding은 측정됨. P1/P2/P4 학습시간은 pilot 전 0 |
...
> + 동결된 split). C0와 embedding smoke는 닫혔고, 현재 병목은 **matched P1/P2/P4 head pilot →
> full frozen probe**다. GPU1은 현재 가용하지만 아직 성능 수치는 하나도 없다.
