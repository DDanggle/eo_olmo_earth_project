...
6. **`decision_experiment_d1_prereg_v0`(2026-09-23 등록 완료)** — §9b의 결정 실험. 단계 H(사람 판독 ≤10인시,
   합의율 ≥.60 + 세 범주 존재) → L(근거 손실 tierA/B/C, K∈{2,3,4,6}) → R(내용 통제, swap은 대체 영상 자체
   정답으로 채점, content_use ≥.30 CI 0 제외). 실행 코드 `code/sn7_evidence_loss_v0.py`,
   `code/sn7_d1_reader_run_v0.py`(각각 자체 테스트 통과). 결과 네 갈래 결정표 포함.
...
## 9b. 재정렬 (2026-09-23) — 산으로 가지 않기 위한 한 가지 결정 실험

문제 진단: "기억을 연구하기 위해 reader를 만들고, reader를 만들기 위해 학습 자료를 다시 만드는" 구조가 됐음. 선행 작업이 본체보다 커짐. MS-155의 "SFT 선행" 결론은 철회함(원인 미확정, silver 목표 불일치).

바뀌지 않는 것: 연구 질문(같은 지역 관측이 쌓일 때 제한된 기억으로 무엇이 변했고 언제 확인됐고 지금 무엇을 모르는지를 근거와 함께 답하기). 논문 목적은 이 질문이고, 기억 모듈 발명이 아님.

이번 한 번의 결정 실험(v0.5 팩 12영상열, 사람 ≤10h 상한):
1. 힌트 없는 독립 판독으로 변화·무변화·정보부족과 뒷받침 관측을 확인(목표 정의 = "제공된 관측 중 변화가 처음 분명히 보이는 관측"; 여러 유효 근거 조합 인정; "변화 있음"과 "그때가 최초"의 근거를 구분).
2. VLM 없이: 최신-K·균등-K·변화량 top-K가 사람이 확인한 근거를 보존하는 비율(근거 손실 측정).
3. 같은 항목에서 대체 영상 자체의 정답을 쓰는 내용 통제로 기존 VLM 2종의 판독을 시험(기존 답 재채점이 아니라 재실행).

| 관측 | 다음 결정 |
|---|---|
| 단순 선택으로도 근거가 충분히 남음 | 이 자료에서 새 기억 방법 투자 근거 약함 → 벤치마크·판독 논문으로 축소 |
| 단순 선택은 근거를 잃고 VLM은 유효 근거를 읽음 | 기억 방법 연구 진입(온라인 갱신, 미래·질문 차단) |
| 근거 손실은 있으나 VLM 판독 불확실 | reader 정렬은 별도 비용의 선택지; 목표 일치 감독·지역 분리 평가를 갖춘 제한 pilot 한 번만 |
| 사람 판독·근거 충분성부터 불명확 | 현재 질문·자료 조합으로 진단 불가 → 닫음 |

이 표는 보편 결론용이 아니라 다음 시간·자원 배분 결정용임. SFT·새 데이터셋·새 아키텍처·전문가 학습을 동시에 붙이지 않음.
CVPR 현실: 마감까지 도달 가능한 것은 이 결정 실험 + (통과 시) 기억 방법의 첫 pilot까지. 방법 주장 논문은 그 뒤 학회 후보. 이 판단을 문서에 남기고 무리하게 묶지 않음.
...
4. SpaceNet 7 서버 취득: **승인됨(2026-09-22)**, 완료(train 60 AOI, 서버 31G).
5. 인수인계 문서 `docs/HANDOFF_2026_09_23.md` 작성 완료 — 다른 기기에서 이어서 할 때 이 문서부터 읽음.
