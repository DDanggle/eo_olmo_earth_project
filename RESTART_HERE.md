...
1. **Presto matched control**: 16/64/256-pixel cache smoke → 6,834 sample cache seal →
   같은 IDs·decoder·3 seed 비교.
2. **Korea preflight**: M10 spatial split seal을 유지하고, AI-Hub S2 v1의 0-fill 오염을 재사용하지
   않는 canonical input/mosaic 계약을 닫는다.
3. **Korea untouched transfer**: P4/P2/P3/C1을 모두 등록한 뒤 test를 한 번만 연다.
4. **Upstream evidence**: 공식 Ai2 저장소의 sample annotation schema PR 후보를 별도 checkout에서
   검증·정리한다.
