// 사람 판독 라벨 — 상위 20곳의 "정답 데이터" 를 만드는 가장 짧은 경로.
...
// 라벨은 임베딩 점수를 *설명* 하지 않는다. 사람이 항공사진·전후 프레임에서 본 것을 4갈래로만 기록한다.

export type LabelCode = 'a' | 'b' | 'c' | 'd';
export type Label = { code: LabelCode; note: string; at: string };
export type LabelFile = { schema: 'oreum-labels/1'; contract_sha256: string | null; exported_at: string; labels: Record<string, Label> };

export const LABEL_CODES: { code: LabelCode; short: string; long: string }[] = [
  { code: 'a', short: '실질 변화', long: '전후 프레임·항공사진에서 지표 변화(나지·시설·식생 손실)가 보인다' },
  { code: 'b', short: '지속 인공물', long: '두 해 모두 같은 시설·도로가 있다 — 연간 변화가 아니다' },
  { code: 'c', short: '대기·구름', long: '구름 가장자리·해무·계절 차이로 깃발이 섰다 — 지표 변화 아님' },
  { code: 'd', short: '판독 불가', long: '프레임·항공사진으로는 판단할 수 없다' },
];
export const LABEL_TOP_N = 20;   // 라벨 대상: 40 m 순위 상위 20
const KEY = 'oreum-labels-v8';
...
export function toFile(labels: Record<string, Label>, contract: string | null): LabelFile {
  return { schema: 'oreum-labels/1', contract_sha256: contract, exported_at: new Date().toISOString(), labels };
}
