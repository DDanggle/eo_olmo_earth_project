...
## M32. P4 열세의 원인은 **캐시 해상도가 아니다** — 두 가설 모두 실측으로 기각됨
...
**천장이 실제 성능의 약 4배임.** P4는 천장 근처에 있지도 않음.
따라서 **"40 m 토큰이라 세밀 경계를 못 그린다"는 설명은 이 데이터에서 성립하지 않음.**
남는 용의자는 (a) 4×64 타일링으로 잘린 문맥, (b) 마지막 layer 하나만 쓰는 237K decoder,
(c) representation adaptation 부재임.

### B. 산사태 크기별 격차 — "가는 흉터에서만 진다" 가설 **기각**
...
격차 최대 지점은 **중간 크기**이고, 가장 작은 산사태에서는 두 방식이 거의 같음
(둘 다 매우 나쁨). 즉 **high-frequency residual이 필요하다는 직접 근거는 아직 없음.**
중간 크기에서 벌어지는 양상은 세밀 경계보다 **객체 단위 문맥**에 가깝고,
64 crop seam 가설과 방향이 맞음.

### 이 진단이 바꾸는 것

E1(원인 분해) 실험의 우선순위가 바뀜.
`1×128 통짜 문맥` → `강한 U-Net decoder(다중 스케일)` → `부분 unfreeze/LoRA` 순서가
`고해상도 residual`보다 먼저임. residual은 A·B가 다시 지지할 때 올림.
...
- 크기 구간은 면적(px)만 씀. **폭·경계복잡도**는 아직 안 씀 — 가는 형태 가설을
  완전히 닫으려면 형태 지표가 필요함.
- 천장 계산은 라벨을 40 m로 내리는 최선 경우임. 실제 캐시가 그 정보를 보존한다는 증명은 아님.
...
## M34. crop 문맥 절단이 **실재함** — 그러나 seam만의 문제가 아니었음
...
**타일 캐시는 crop 경계에서 이웃 토큰이 내부보다 49% 더 이질적임 — 인공 단절이 실재함.**
통짜 인코딩은 0.82로, 경계가 내부보다 오히려 매끄러움(실제 지형이 연속이므로 정상).
...
**따라서 M31의 "성공 2,539 / 94.1%"를 철회함.** 실사용 가능은 1% 기준 **1,912개(75.3%)**임.
...
전자가 데이터를 더 살리고 후자가 단순함. **아직 고르지 않았고, 고르기 전에 재실행하지 않음.**
