...
| | 방식 A (공유 캐시) | 방식 B (직접 학습) |
|---|---|---|
| 과제 1개 | 1,130 + 641 = **1,771 s** | **1,491 s** ← A가 19% 비쌈 |
| 과제 3개 | 1,130 + 3×641 = **3,053 s** | 3×1,491 = **4,473 s** ← A 유리 |

즉 **공유 캐시의 비용 우위는 과제 3개 이상에서만 발생함.** 단일 과제에서는 오히려 손해임.

### 아직 말할 수 없는 것

- **신뢰구간이 없음.** IoU 격차 0.028712가 지역·event 단위 짝지은 부트스트랩에서
  살아남는지 미측정. 이긴 쪽 숫자도 구간 없이는 확정이 아님.
- **조기종료 규칙이 방식마다 통일되지 않았음.** P2는 18에서 best인데 40까지 강제했음.
  동일한 val-only 조기종료 규칙을 전 방식에 적용해야 공정함. **미해결 결함으로 기록함.**
